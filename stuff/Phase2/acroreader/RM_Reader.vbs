...
Function UninstallApp(strDisplayName, strVersion, strID, strUninstall)
    ...
    Else 'We have the GUID
        Uninstall = """C:\Windows\system32\MsiExec.exe"" /X " & strID & " /l*v " & chr(34) & "UNINST_" & strDisplayName & " v" & strVersion & ".log" & Chr(34) & " /qn /norestart "
    End If
...
Sub UninstallLoop(strKeyPath)
    ...
    For Each Product In arrSubKeys
        ...
        If InStr(1,UCase(strName),UCase("Reader"),1) > 0 AND InStr(1,UCase(strID),UCase("7B44"),1) > 0 Then
           UninstallApp strDisplayName, strVersion, strID, strUninstall
        End If

        'Remove Acrobat Pro - 7760

'        If InStr(1,UCase(strName),UCase("Acrobat"),1) > 0 AND InStr(1,UCase(strID),UCase("7760"),1) > 0 Then
'            UninstallApp strDisplayName, strVersion, strID, strUninstall
'        End If

        'Remove Acrobat Standard - BA7E

'        If InStr(1,UCase(strName),UCase("Acrobat"),1) > 0 AND InStr(1,UCase(strID),UCase("BA7E"),1) > 0 Then
'            UninstallApp strDisplayName, strVersion, strID, strUninstall
'        End If

        'Remove Acrobat.com

'        If InStr(1,UCase(strName),UCase("Acrobat.com"),1) > 0 Then
'            UninstallApp strDisplayName, strVersion, strID, strUninstall
'        End If
