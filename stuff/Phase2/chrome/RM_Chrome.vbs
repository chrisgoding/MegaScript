...
'
'Purpose:      The following script will remove MSI applications.
'
...
Function UninstallApp(strDisplayName, strVersion, strID, strUninstall)
    ...
    Else 'We have the GUID
                Uninstall = """C:\Windows\system32\MsiExec.exe"" /X " & strID & " /l*v " & chr(34) & "c:UINST_" & strDisplayName & " v" & strVersion & ".log" & Chr(34) & " /qn /norestart "
    End If
...
WScript.Echo ""
WScript.Echo Now() & " - Searching for Application..."
...
Sub UninstallLoop(strKeyPath)
    ...
    For Each Product In arrSubKeys
        ...
        'Java has used many different Client names throughout the years.  This
        'should capture most, if not all, of them.

        If InStr(1,UCase(strName),UCase("Google Chrome"),1) > 0 Then
